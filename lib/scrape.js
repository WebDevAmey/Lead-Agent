...
const PROCESS_PHRASES = [
  'hand-sewn', 'hand sewn', 'handmade', 'hand-made', 'handcrafted', 'hand-crafted',
  'hand-poured', 'hand poured', 'hand-painted', 'hand painted', 'hand-knit', 'hand knit',
  'small batch', 'made to order', 'by hand', 'in my studio',
];

// Builds a short, genuine-sounding compliment from whatever the about page
// reveals — falls back to store name + product count when nothing specific
// is found.
export function generateCompliment(signals, storeName) {
  const aboutText = (signals.aboutText || '').toLowerCase();

  for (const phrase of PROCESS_PHRASES) {
    if (aboutText.includes(phrase)) {
      return `Your ${phrase} approach to making products really stands out — that kind of care is rare to see.`;
    }
  }

  if (signals.founderName) {
    return `The founder story on your about page is genuinely refreshing — making each piece yourself is rare.`;
  }

  if (signals.usAddress) {
    return `Love that you're a US-based small business — that personal touch really comes through on your site.`;
  }

  const name = storeName || 'Your store';
  const products = signals.productCount;
  if (products) {
    return `${name} has a great focused range of ${products} products that feels very intentional.`;
  }
  return `${name} has a great focused product range that feels very intentional.`;
}
...
export async function scrapeStore(domain) {
  ...
  for (const path of CRAWL_PATHS) {
    ...
    const $ = load(res.text);
    const pageText = processPage($, res.text, signals);
    allText += pageText + ' ';
    ...
    if (!signals.aboutUrl && ABOUT_PATHS.includes(path)) {
      signals.aboutUrl = res.url;
      signals.aboutText = pageText;
    }
  ...
  signals.compliment = generateCompliment(signals, signals.storeName);
